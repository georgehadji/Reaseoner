...
try:
    ...
    logger.warning("mathjs not installed - calculation widget disabled")
...
def calculate_expression(expression: str) -> dict[str, Any]:
    ...
    if not MATHJS_AVAILABLE:
        # Fallback to basic eval (limited functionality)
        try:
            # Only allow safe characters
            allowed = set("0123456789+-*/.() ")
            if not all(c in allowed for c in expression):
                return {"error": "Invalid characters", "valid": False}
...
try:
    ...
    try:
        ...
        logger.warning("yahooquery/yfinance not installed - stock widget disabled")
...
def get_discover_content(topic: str = "tech", mode: str = "normal") -> dict[str, Any]:
    """
    ...
    for query in topic_config["queries"]:
        # Try SearXNG
        searxng_results = asyncio.run(search_searxng(query, ["bing news"]))
